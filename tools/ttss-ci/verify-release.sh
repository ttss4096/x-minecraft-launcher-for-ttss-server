...
jq -e '.schemaVersion == 1 and (.releaseId | test("^ttss-launcher-[0-9a-f]{20}$")) and (.seed.contentDigest | test("^[0-9a-f]{64}$")) and (.seed.payloadSha256 | test("^[0-9a-f]{64}$")) and (.seed.payloadSource | test("^xmcl/[^#]+#\\./resources/ttss-client-seed/ttss-client-seed\\.zip$")) and (.inputs.hostManifestSha256 | test("^[0-9a-f]{64}$")) and (.inputs.hailwallSha256 | test("^[0-9a-f]{64}$")) and (.inputs.policySha256 | test("^[0-9a-f]{64}$"))' "$release/release-manifest.json" >/dev/null
...
asar="$release/xmcl/app-$xmcl_version-linux.asar"
expected_asar=$(tr -d '\r\n' < "$asar.sha256")
...
archive="$release/xmcl/ttss-xmcl-$xmcl_version-linux-x64.tar.gz"
gzip -t "$archive"
...
tar -xOf "$archive" './resources/ttss-client-seed/ttss-client-seed.zip' > "$work/ttss-client-seed.zip"
for name in ttss-client-seed.manifest.json ttss-client-seed.manifest.sig ttss-client-seed.public.pem; do
  tar -xOf "$archive" "./resources/ttss-client-seed/$name" | cmp - "$seed/$name"
done
